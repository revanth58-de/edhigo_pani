  ...
  el.innerHTML = `
    <div class="section-header">
      <div>
        ...
        <div class="section-sub">Configure wages, rental commissions, location telemetry parameters, and core mobile client features.</div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1.2fr 0.8fr;gap:20px;margin-bottom:20px">
      ...
      <div class="card" style="padding:24px;display:flex;flex-direction:column;justify-content:space-between">
        <div style="font-size:16px;font-weight:800;color:var(--danger);margin-bottom:16px;display:flex;align-items:center;gap:8px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:12px;background:rgba(255,255,255,0.02);border:1px solid var(--glass-border);margin-top:12px">
          <div>
...
async function loadData() {
  try {
    ...
    document.getElementById('rentPump').value = rentRates.Pump || '';
    ...
    document.getElementById('notificationsEnabled').checked = settingsData['app.notificationsEnabled'] === 'true';
  ...
  try {
    ...
    const updates = {
      ...
      'rents.machineryBaseRates': JSON.stringify({
        ...
        Pump: parseInt(document.getElementById('rentPump').value) || 200
      }),
