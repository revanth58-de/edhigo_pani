 ...
 */
const getApiUrl = () => {
  if (Platform.OS === 'web') return 'http://127.0.0.1:5000/api'; // Browser on same PC
 ...
 */
const getSocketUrl = () => {
  if (Platform.OS === 'web') return 'http://127.0.0.1:5000'; // Browser on same PC
  return `http://${LOCAL_IP}:5000`;  // Phone must be on same WiFi
};
